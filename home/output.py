...
Original file is located at
    https://colab.research.google.com/drive/12TRIUVkXNyy9-vHj6isYRIcHraves2pu
...
bd=data[[ 'diabetes','chd', 'htn', 'cancer', 'asthma', 'copd', 'autoimmune_dis', 'smoker','high_risk_exposure_occupation', 'high_risk_interactions','rapid_flu_results',
       ...
       'sore_throat', 'er_referral']]
cd=data[[ 'test_name', 'swab_type', 'covid19_test_results', 
        'is_abnormal_cxr']]
nd=data[[ 'age','temperature', 'pulse', 'sys', 'dia', 'rr', 'sats','days_since_symptom_onset', 'num_symptoms','severity_score']]
label=data[['sym_severity']]

cd=pd.get_dummies(cd)
bd=bd.astype(object)
bd=pd.get_dummies(bd)

X=pd.concat([cd,bd,nd],axis=1)
...
def build_classifier(optimzer):
    ...
    classifier.add(Dense(6, input_dim = 106, kernel_initializer = 'glorot_uniform', activation = 'relu' ))
